...
public class PreScoringValidationServiceImpl implements PreScoringValidationService {
    ...
    private void isMiddleNameValid(String middleName) {
        if (middleName != null) {
            if (middleName.length() > validationProperties.getNameMaxSymbolsRestriction()) {
            ...
            } else if (!middleName.matches(validationProperties.getNamesRegex())) {
    ...
    private void isFirstNameValid(String firstName) {
        if (firstName.length() > validationProperties.getNameMaxSymbolsRestriction()) {
        ...
        (!firstName.matches(validationProperties.getNamesRegex())) {
    ...
    private void isLastNameValid(String lastName) {
        if (lastName.length() > validationProperties.getNameMaxSymbolsRestriction()) {
            ...
            throw new ValidationException("Минимальная длина отчества составляет "
                    + validationProperties.getNameMinSymbolsRestriction() + " символа.");
        } else if (!lastName.matches(validationProperties.getNamesRegex())) {
    ...
    private void isPassportSeriesValid(String passportSeries) {
        if (passportSeries.length() > validationProperties.getCountOfPassportSeriesDigits()) {
            log.info("серия паспорта {} больше требуемого количества цифр", passportSeries);
        ...
        } else if (passportSeries.length() < validationProperties.getCountOfPassportSeriesDigits()) {
            log.info("серия паспорта {} меньше требуемого количества цифр", passportSeries);
            throw new ValidationException("Минимальная длина серии паспорта составляет "
                    + validationProperties.getCountOfPassportSeriesDigits() + " цифр.");
        } else if (!passportSeries.matches(validationProperties.getPassportSeriesRegex())) {
    ...
    private void isPassportNumberValid(String passportNumber) {
        if (passportNumber.length() != validationProperties.getCountOfPassportNumberDigits()) {
